...
interface IptvDao {
    ...
    @Query("SELECT DISTINCT groupTitle FROM channels WHERE playlistId = :playlistId AND groupTitle IS NOT NULL AND groupTitle != '' ORDER BY groupTitle ASC")
    fun getGroupTitlesForPlaylist(playlistId: Int): Flow<List<String>>

    @Query("SELECT * FROM channels WHERE playlistId = :playlistId AND (:groupTitle IS NULL OR groupTitle = :groupTitle) ORDER BY name ASC")
